...
interface HistogramEngineState {
  readonly aggregationProvider: HistogramEngineBackend['aggregationProvider'];
...
export function createHistogramEngine(
  hostElement: HTMLElement,
  options: HistogramPlotOptions,
  backend: HistogramEngineBackend,
): HistogramPlotInstance {
  ...
  let disposed = false;
  let state = createInitialState(options, backend);
  ...
  if (state.options.hostClassName ?? backend.hostClassName) {
    hostElement.classList.add(
      ...splitClassNames(state.options.hostClassName ?? backend.hostClassName),
    );
  }
  hostElement.style.position ||= 'relative';

  const canvas = document.createElement('canvas');
  canvas.className = state.options.canvasClassName ?? backend.canvasClassName;
  canvas.setAttribute('aria-label', state.options.canvasLabel ?? backend.canvasLabel);
  ...
  disposables.defer(() => {
    ...
    backend.aggregationProvider.dispose();
    destroyRenderer();
    canvas.remove();
  ...
  function buildRendererOptions(): HistogramRendererOptions {
    return {
  ...
  function createRenderer(): void {
    ...
    backend.assertAvailable?.(state.options);
    renderer = backend.createRenderer(buildRendererOptions(), state.options, {
      ...
      onError(error) {
        handleSetupError(error, backend.setupErrorMessage ?? 'Unknown histogram renderer error.');
      },
      onReady() {
        emitRenderState('ready');
      },
      ...
      backend.attachContextLifecycle(canvas, {
        ...
        onRestored(event) {
          ...
          try {
            ...
            handleSetupError(
              error,
              backend.contextRestoreErrorMessage ??
                'Unknown histogram renderer restore error.',
            );
          }
        },
      }),
  ...
  const commands: HistogramPlotCommands = {
    ...
    materializeSelectionSourceIndices() {
      ...
      state = rebuildStateAggregation(state, {
        includeMembership:
          !state.deferMembership &&
          !aggregationHasPendingMembership(state.aggregation),
      });
      renderer?.update({
        aggregation: state.aggregation,
        selectedBinKeys: getRenderableSelectedBinKeys(state),
    ...
    setViewport(
      ...
      const aggregationViewport = state.options.viewport ?? state.viewport;
      const aggregationDependsOnViewport =
        shouldUseViewportBoundAggregation(state) &&
        viewportChangeAffectsAggregation(aggregationViewport, normalizedViewport);
      if (phase === 'commit' && aggregationDependsOnViewport) {
        state = rebuildStateAggregation(state, {
          includeMembership: reason === 'programmatic' && !state.deferMembership,
  ...
  try {
    ...
    handleSetupError(error, backend.setupErrorMessage ?? 'Unknown histogram renderer setup error.');
  }
...
function createInitialState(
  ...
  const preparedAggregationState = createPreparedAggregationState(
    options,
    backend.aggregationProvider,
  );
  const aggregation =
    options.aggregation ??
    ...
    }, backend.aggregationProvider, backend.deferMembership === true);
  const viewport = normalizeHistogramViewport(
    options.viewport,
    createDefaultHistogramViewport(aggregation),
  );
  return {
    aggregationProvider: backend.aggregationProvider,
    aggregation,
    lastSelection: null,
    preparedAggregationState,
    binSizes,
    deferMembership: backend.deferMembership === true,
    options: {
      ...options,
      aggregation,
      binSizes,
...
function reconcileState(
  ...
  const columnsChanged =
    ...
    partialOptions.columns !== previous.options.columns;
  const specChanged =
    partialOptions.spec !== undefined &&
    partialOptions.spec !== previous.options.spec;
  const preparedAggregationState =
    columnsChanged || specChanged
      ? createPreparedAggregationState(options, previous.aggregationProvider)
  ...
  let nextState: HistogramEngineState = {
    aggregationProvider: previous.aggregationProvider,
    aggregation: aggregationOverride ?? previous.aggregation,
    binSizes,
    deferMembership: previous.deferMembership,
  ...
  if (needsAggregationRebuild) {
    nextState = rebuildStateAggregation(nextState, {
      includeMembership: !nextState.deferMembership,
      viewport: provisionalViewport,
    });
  }
...
function buildAggregationFromOptions(
  ...
  aggregationProvider: HistogramEngineBackend['aggregationProvider'],
  deferMembership: boolean,
  ...
  return withAggregateBuildMs(
    aggregationProvider.build(options.columns, {
      binSizes: options.binSizes,
      hoverSourceIndex: options.hoverSourceIndex,
      includeMembership: !deferMembership,
...
function createPreparedAggregationState(
  ...
  return aggregationProvider.prepare(options.columns, options.spec);
}
...
function updateStateForBinSizes(
  ...
  if (!viewportChangeAffectsAggregation(provisionalState.viewport, normalizedViewport)) {
    return {
      ...aggregatedState,
      options: {
