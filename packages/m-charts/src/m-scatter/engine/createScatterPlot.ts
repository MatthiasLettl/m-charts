...
import {
  ...
  type FastScatterWebglRendererOptions,
...
const DEFAULT_CANVAS_LABEL = 'WebGL2 scatter-fast point canvas';
const DEFAULT_CANVAS_CLASS = 'scatter-fast-engine-canvas';
const DEFAULT_HOST_CLASS = 'scatter-fast-engine-host';
...
export function createFastScatterPlot(
  ...
  if (options.hostClassName ?? DEFAULT_HOST_CLASS) {
    hostElement.classList.add(...splitClassNames(options.hostClassName ?? DEFAULT_HOST_CLASS));
  }
  hostElement.style.position ||= 'relative';

  const canvas = document.createElement('canvas');
  canvas.className = options.canvasClassName ?? DEFAULT_CANVAS_CLASS;
  canvas.setAttribute('aria-label', options.canvasLabel ?? DEFAULT_CANVAS_LABEL);
  canvas.dataset.renderer = 'webgl2-points';
  ...
  function buildRendererOptions(): FastScatterWebglRendererOptions {
    return {
      ...optionsState,
      canvas,
      onHoverChange: (hover) => {
  ...
  function createRenderer(): void {
    ...
    const rendererFactory =
      ...
      ((rendererOptions) => new FastScatterWebglRenderer(rendererOptions));
    ...
    createWebGlContextLifecycle(canvas, {
      ...
      onRestored(event) {
        ...
        try {
          ...
          handleSetupError(error, 'Unknown scatter-fast WebGL2 renderer restore error.');
        }
      },
  ...
  const commands: FastScatterPlotCommands = {
    ...
    getNavigatorWindowPixels(widthCssPx) {
      return calculateFastScatterNavigatorWindowPixels(
        ...
        calculateFastScatterDomain(optionsState.columns, optionsState.spec).x,
        widthCssPx,
      );
    },
    getRenderSnapshot() {
    ...
    dragNavigator(request) {
      ...
      const startedAt = performance.now();
      const domain = calculateFastScatterDomain(optionsState.columns, optionsState.spec).x;
      const x =
        request.edge === null
          ? dragFastScatterNavigatorWindow({
              currentPointerCssX: request.currentPointerCssX,
  ...
  function collectHeatmapAggregateSelectionInBounds({
    ...
    for (let cellIndex = 0; cellIndex < subplot.cellCount; cellIndex += 1) {
      ...
      selected.push(
        ...materializeFastScatterHeatmapCellSourceIndices(subplot, cellIndex),
      );
    }
    return {
      candidateCount,
      sourceIndices: normalizeSelectionSourceIndices(selected),
    };
  }
  ...
  try {
    ...
    handleSetupError(error, 'Unknown scatter-fast WebGL2 renderer setup error.');
  }
...
function buildRendererUpdateOptions(
  ...
  rendererOptions: FastScatterWebglRendererOptions,
